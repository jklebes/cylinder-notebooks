...
def get_VWmatrix(field, maxwavevector, alpha, n, C, lx,ly, lambda_is):
  lambdas=dict([(i, getlambda(i, field, maxwavevector)) for i in lambda_is])
  n_lambdas=len(lambda_is)
  ...
  #print(Aprime.shape)
  #print(Aprime)
  A=Aprime[:-n_lambdas, :-n_lambdas].round(3)
  ...
  for (i,label) in enumerate(lambda_is):
    for (nx,qx) in enumerate(range(-maxwavevector[0]+1,maxwavevector[0])):
      for (ny,qy) in enumerate(range(-maxwavevector[1]+1,maxwavevector[1])):
        Aprime[(nx)*(2*maxwavevector[1]-1)+ny, (2*maxwavevector[1]-1)*(2*maxwavevector[0]-1)+i] = 1j*(lambdas[label][nx, ny]).conjugate()
        Aprime[(2*maxwavevector[1]-1)*(2*maxwavevector[0]-1)+i,(nx)*(2*maxwavevector[1]-1)+ny] = 1j*(lambdas[label][nx, ny])
  ...
  #print(Aprime)
  print("det A'",np.linalg.det(Aprime))
